...
- Na solicitação/execução de ATA, **Carona** identifica pedidos de outras secretarias que
  ...
  próprio**: não há vínculo com `emendas`. A secretaria solicitante deve ser escolhida
...
- Cada documentação do checklist é cadastrada uma única vez por seção e pode valer para
  todos os contratos de manutenção ou para vários contratos selecionados por meio de
  `nf_checklist_documento_contratos`. A ordem de exibição é atribuída automaticamente pelo
...
- Na subaba **Controle de Entregas / Prazos**, a ordem é: **aguardando AF**, atrasado,
  vence hoje, no prazo, sem prazo, recebido e cancelado. Dentro de cada grupo permanece
  a ordenação por dias restantes, com os mais atrasados primeiro no grupo de atrasos.
