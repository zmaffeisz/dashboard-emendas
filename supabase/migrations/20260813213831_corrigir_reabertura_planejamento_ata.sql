-- Reabre o planejamento antes da verificação da FK ao excluir uma requisição.
...
begin
  if tg_op = 'UPDATE' then
    ...
    if old.status = 'CANCELADO' then raise exception 'Um planejamento cancelado não pode ser reativado.'; end if;
  ...
  if not exists (select 1 from public.emenda_itens ei where ei.id=new.emenda_item_id and ei.emenda_id=new.emenda_id) then
    raise exception 'O item informado não pertence à Emenda.';
  end if;
  if not exists (select 1 from public.itens i where i.id=new.processo_item_id and i.processo_id=new.processo_id and i.origem='ata') then
    raise exception 'O item informado não pertence ao processo de Ata.';
  end if;
  if new.status in ('ATA_VIGENTE_AGUARDANDO_REQUISICAO','REQUISITADO') and not exists (
    select 1 from public.atas_itens ai join public.itens i on i.ata_item_id=ai.id
     where ai.id=new.ata_item_id and ai.contrato_id=new.contrato_id
       and i.processo_id=new.processo_id and i.contrato_id=new.contrato_id
       and (i.id=new.processo_item_id or i.descricao=(select origem.descricao from public.itens origem where origem.id=new.processo_item_id))
  ...
  if new.status='ATA_VIGENTE_AGUARDANDO_REQUISICAO'
     ...
     and exists (select 1 from public.atas_execucao ae where ae.ata_item_id=new.ata_item_id and ae.emenda_item_id=new.emenda_item_id) then
    raise exception 'Já existe uma requisição para este planejamento.';
  end if;
  if new.status='REQUISITADO' and not exists (
    select 1 from public.atas_execucao ae where ae.id=new.ata_execucao_id and ae.ata_item_id=new.ata_item_id
      and ae.emenda_item_id=new.emenda_item_id and ae.emenda_id=new.emenda_id and ae.qtde=new.quantidade_requisitada
